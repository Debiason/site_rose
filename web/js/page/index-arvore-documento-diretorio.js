$(() => {
  const fldArvoreDocumento = $("select[name='arvore_documento']");

  new PerfectScrollbar('#drop_zone')

  $("#btn-mount-tree").on("click", () => {
    const arvore_documento_id = String(fldArvoreDocumento.val()).replace(/\D/, "");
    ...
    $("#body-arvore-doc").load(
      `/arvore-documento-diretorio/tree-doc?id=${arvore_documento_id}`, () => {
        setTimeout(() => {
  ...
  $(document).on('click', "a.jstree-anchor, .box-items", (ev) => {
    ...
    if(!$(`#${element_id}`).hasClass('a-open-file')) {
      ...
      $.ajax({
        url: '/arvore-documento-diretorio/get-tree-documento-interno',
        data: {
          id: element_id
        },
        async: false,
        success: function (body) {
          if (body == ''){
            toastr.error('Permissão para acesso ao diretóro negada.')
          } else {
            ...
            if (treeFilter.length === 0) {
              fetch(`/arvore-documento-diretorio/get-arquivos-diretorio?id=${element_id}`)
...
function drag_drop(ev) {
  ...
  try{
      fetch(`/arvore-documento-diretorio/existe-lista-vinculada?id=${arvore_documento_id}`)
        .then((response) => response.json())
        .then((body) => {
          if(body.success && !body.has_directory_child) {
            ...
            formAnexo.append('nome_lista', text);
            ...
            $.ajax({
              type: 'POST',
              url: '/arvore-documento-diretorio/salvar-anexo',
              data: formAnexo,
              dataType: 'json',
              cache : false,
              processData: false,
